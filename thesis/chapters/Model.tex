...
\subsection{Vote validation}
\label{subsec:DiscussionTwoPhaseCommit}

While the model does not include the cryptographic elements of the judgment algorithm, it follows the idea of adding and revealing votes in two different steps and instead assumes the mechanism for validating the revealed votes to always work.

% \textbf{(Do I need a correctness argument in my results for the cryptographic stuff?)} Additionally, the model does not use per package judgment unique values for positive and negative votes as described in section \ref{subsec:AlgorithmSingle}. While such additions would be needed for the security properties of the system, they are not problematic to simplify in the model as they do not change the possible behavior of the system actors.

\subsection{System-actor behavior}
