...
\begin{description}
	...
	\item[Liveness] Every \texttt{.buildinfo} file is eventually judged.
\end{description}
...
Given a set of \texttt{.buildinfo} files $Files$, a mapping from files to \textit{valid} judgments $Validity: File \mapsto Boolean$ and a mapping from files to system judgments $Judgement: File \mapsto Boolean$, the correctness of the system is defined as

\begin{description}
	\item[Safety] $\forall f \in range(Judgment): Judgment[f] = Validity[f]$
	\item[Liveness] $\Diamond\Box(\forall f \in Files: f \in range(Judgment))$ \textbf{(is this really what you mean?)(Use labels like in an equation)}
\end{description}
